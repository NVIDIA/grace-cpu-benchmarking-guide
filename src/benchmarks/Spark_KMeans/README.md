# HiBench Benchmark Suite: Kmeans

This workload from the HiBench suite tests the K-means (a well-known clustering algorithm for knowledge discovery and data mining) clustering in spark.mllib. The input data set is generated by GenKMeansDataset based on Uniform Distribution and Guassian Distribution. There is also an optimized K-means implementation based on DAL (Intel Data Analytics Library), which is available in the dal module of sparkbench.

## Initial Configuration

In order to run HiBench Kmeans-Spark test, we need to install HiBench, Hadoop and Spark. HiBench is the wokload generator. Hadoop is used to generate data and serve as file system to store the data. Before you run HiBench test, you need to have hadoop up and running. Otherwise, your test will fail. Spark is the targeted application to run test with. The following notes break down the installation and document fixes to some possible issues.
Spark	3.3.0	jdk 11.0	compatible with Hibench
HiBench	7.1.1	jdk 8.0	To build, you will need jdk 8.0 and spark 2.4 scale 2.11
Hadoop	3.3.3	jdk 11.0	compatible with Hibench
## Quick Start

Install HiBench first

HiBench is an open source benchmark for big data application. As of today when this page is created, latest version is Current version: 7.1.1. It works well with latest Spark and Hadoop version. But in order to install and compile it, we need to use java 8. Newer Java versions are not compatible with this setup.
In order to make maven work, you have to use "-Dspark=2.4 -Dscala=2.11". After the installation is done, you can switch to your latest/other version of Spark in your conf/spark.conf file. 
...
Edit the bashrc file, and source It
```bash
export JAVA_HOME="/usr/lib/jvm/java-11-openjdk-arm64"
export PATH="$JAVA_HOME/bin:$PATH"
...
Make changes to the following files based on your installation paths Hadoop, Spark and HiBench.
hadoop.conf
```bash
# Hadoop home
...
Example output of what the command jps should look like if the above command was successful.
```bash
<PROMPT>$ jps
...
Script to run spark kmeans on a 72 core Grace system
```bash
#!/usr/bin/bash
...
The results can be found in $HIBENCH_DIR/report/hibench.report.


## Output

Expected output on Grace:
```bash
