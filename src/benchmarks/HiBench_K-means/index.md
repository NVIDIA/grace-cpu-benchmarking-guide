...
* `hdfs-site.xml`
    ...
    <value>$PATH_TOO_HADOOP/namenode</value>
    </property>
    <property>
...
```admonish info
Java 8 is required to build HiBench.  If the `mvn` command fails with an error like `error: scala.reflect.internal.MissingRequirementError: object java.lang.Object in compiler mirror not found.` please check that you have installed Java 8 and updated your `JAVA_HOME` and `PATH` environment variables.
```

Once HiBench builds successfully, configure your environment with Java 11 and the newly-installed Hadoop:
...
sed -i 's#/PATH/TO/YOUR/SPARK/HOME#/home/nvidia/spark-3.3.0-bin-hadoop3#g' spark.conf
```
...
2. Run Spark k-means:
    ...
    numactl -C0-71 -N1 -m0 $HOME/HiBench/bin/workloads/ml/kmeans/spark/run.sh
    ```
